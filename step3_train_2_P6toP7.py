...
parser.add_argument('--data_dir', type=str, default= 'data/',
                    help='data file name containing the discrete files.')
parser.add_argument('--data_sample_size', type=int, default=1191,#5000,1233,1191
                    help='the number of samples of data')
parser.add_argument('--data_variable_size', type=int, default=94,#10,102,94
...
parser.add_argument('--x_dims', type=int, default=1,#1 #changed here
                    help='The number of input dimensions: default 1.')
parser.add_argument('--z_dims', type=int, default=1,#1
                    help='The number of latent variable dimensions: default the same as variable size.')
parser.add_argument('--path', type=str, default='data/GSE104276_all_pfc_2394_UMI_TPM_NOERCC_P6toP7_preprocessing_deg.csv',
                    help='The path of real gene data.')
...
parser.add_argument('--epoch_cnt', type=int, default= 0,
                    help='Number of all epochs to train.')
...
# get data: 
train_loader, valid_loader, test_loader = load_gene_data(args, args.batch_size, args.suffix)
...
if args.encoder == 'mlp':
    ...
    encoder_con = MLPEncoder(args.data_variable_size * args.x_dims, args.x_dims, args.encoder_hidden,
                         int(args.z_dims), adj_A,
                         batch_size = args.batch_size,
                         do_prob = args.encoder_dropout, factor = args.factor).double()
...
if args.load_folder:
    ...
    encoder_con_file = os.path.join(args.load_folder, 'encoder_con.pt')
    encoder_con.load_state_dict(torch.load(encoder_con_file))
...
if args.optimizer == 'Adam':
    optimizer = optim.Adam(list(encoder.parameters()) + list(encoder_con.parameters()) + list(decoder.parameters()),lr=args.lr)
...
def train(epoch, best_val_loss, lambda_A, c_A, optimizer):
    ...
    for batch_idx, (data, relations, data_con) in enumerate(train_loader):
        
        if args.cuda:
            data, relations, data_con = data.cuda(), relations.cuda(), data_con.cuda()
        data, relations, data_con = Variable(data).double(), Variable(relations).double(), Variable(data_con).double()
        ...
        # contrastive learning
        # data augmentation - gauss noise - random.gauss(0,0.05)
        # data_con = data        
        # for i in range(data_con.shape[0]):
        #    for j in range(data_con.shape[1]):
        #        gaunoise = random.gauss(0,0.05)
        #        data_con[i,j,0] = torch.abs(data_con[i,j,0]+gaunoise)
        ...
        enc_x_, logits_, origin_A_, adj_A_tilt_encoder_, z_gap_, z_positive_, myA_, Wa_ = encoder_con(data_con, rel_rec, rel_send)  # logits is of size: [num_sims, z_dims]
        ...
        # contrastive loss - L2 or MSE
        loss_con = con_mse(logits,logits_)
        
        # ELBO loss:
        loss = loss_kl + loss_nll + loss_con
...
if __name__ == '__main__':
    ...
    # test()
    print("args.epoch_cnt:", args.epoch_cnt)
    graph_ = origin_A.data.clone().numpy()
    np.save('graph_epoch_'+str(args.epoch_cnt),graph_)
    graph_ = np.load('graph_epoch_'+str(args.epoch_cnt)+'.npy')
    
    # obtain BN matrix     
    # weight = W * W.T
    graph_t = graph_.copy()
    graph_t = np.dot(graph_t,graph_t.T)
    # make the diagonal elements be 0
    diag = np.diag(graph_t)
    diag_t = np.diag(diag)
    graph_tt = graph_t - diag_t

    thr_ = np.mean(abs(graph_[:,0])) #0.3
    graph_tmp = graph_.copy()
    graph_tmp[np.abs(graph_tmp) < thr_] = 0
